...
vae = VariationalAutoencoder(next(iter(dataloader))[1].shape, 500) # second parameter is number of latent features that the model whas trained on (backed up model is 100)
vae.load_state_dict(torch.load('vae_LF_500_Epochs_100.pth', map_location=torch.device('cpu')))
...
Zs_df.to_csv('VAE_500_100_pre.tsv.gz', sep='\t', index=False, compression="gzip")
