...
class VariationalAutoencoder(nn.Module):
    ...
    * a Gaussian posterior `q_\phi(z|x) = N(z | \mu(x), \sigma(x))`
    ...
    def observation_model(self, z:Tensor) -> Distribution:
        ...
        # Parameters for TruncatedNormal: Assuming mean and std are derived from px_params
        # and you have defined reasonable truncation limits (a, b)
        mean = px_params  # Or some transformation of px_params
        std = torch.ones_like(px_params)  # Or some transformation of px_params
        a, b = 0., 21.

        return TruncatedNormal(mean, std, a, b)  # Using TruncatedNormal
